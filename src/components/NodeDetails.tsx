  ...
  return (
    ...
    >
      ...
      >
        ...
        {(isServer || isDb) && (
          <div style={{ display: "flex", alignItems: "center", gap: "16px", background: "var(--ui-bg)", padding: "14px", borderRadius: "12px", border: "1px solid var(--ui-line)" }}>
            <svg width="70" height="70" viewBox="0 0 80 80">
              <circle cx="40" cy="40" r="28" fill="none" stroke="var(--ui-line-strong)" strokeWidth="6" />
            ...
            <div style={{ display: "flex", flexDirection: "column", justifyContent: "center" }}>
              <span style={{ fontSize: "10px", color: "var(--ui-muted)", fontWeight: 800 }}>QUEUE STATUS</span>
              <div style={{ fontSize: "20px", fontWeight: 800, color: "var(--ui-text)" }}>
                {currentQueue} <span style={{ fontSize: "13px", color: "var(--ui-muted)", fontWeight: 500 }}>/ {maxQueue}</span>
              </div>
            </div>
          </div>
        )}
        ...
        {(isServer || isDb) && (
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            <span style={{ fontSize: "10px", color: "var(--ui-blue)", fontWeight: 800, letterSpacing: "0.05em" }}>EQUIPPED MODULE SLOTS</span>
                    ...
                    {moduleType ? (
                      <>
                        <div style={{ display: "flex", flexDirection: "column", textAlign: "left" }}>
                          ...
                          <span style={{ fontSize: "8px", color: "var(--ui-muted)" }}>Slot {slotIdx + 1}</span>
                        </div>
                        <button
                          type="button"
                      ...
                      <span style={{ color: "var(--ui-muted)", fontSize: "11px" }}>Empty Slot</span>
                    )}
                  </div>
                );
              })}
        ...
        <div className="node-details-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", fontSize: "13px" }}>
          <span style={{ display: "grid", gap: "2px", padding: "10px", borderRadius: "10px", background: "var(--ui-bg)", border: "1px solid var(--ui-line)" }}>
            <small style={{ display: "block", fontSize: "9px", color: "var(--ui-muted)", fontWeight: 800 }}>POSITION</small>
          ...
          <span style={{ display: "grid", gap: "2px", padding: "10px", borderRadius: "10px", background: "var(--ui-bg)", border: "1px solid var(--ui-line)" }}>
            <small style={{ display: "block", fontSize: "9px", color: "var(--ui-muted)", fontWeight: 800 }}>LINKS</small>
            <strong style={{ color: "var(--ui-text)" }}>
        ...
        <ul style={{ listStyle: "none", fontSize: "12px", display: "flex", flexDirection: "column", gap: "6px", borderTop: "1px solid var(--ui-line)", paddingTop: "12px", paddingLeft: 0, margin: 0 }}>
          {portLimits.traffic > 0 && (
            <li style={{ color: "var(--ui-text)", padding: "4px 8px", background: "var(--ui-bg)", borderRadius: "6px" }}>
          ...
          {copy.stats.map((stat) => (
            <li key={stat} style={{ color: "var(--ui-muted)", paddingLeft: "8px" }}>• {stat}</li>
          ))}
        </ul>
        <small className="node-details-hint" style={{ fontSize: "10px", color: "var(--ui-muted)" }}>
