...
플레이어가 상점에서 장비를 구매하고 격자에 배치한 뒤 링크를 연결한다. 트래픽 입구와 응답 출구는 상단 고정 시설이다.
...
트래픽 입구와 응답 출구는 구매 장비가 아니며 보드 상단에 고정된다. 최초 보유 장비는 없다.

최초 Wave는 구매·배치·연결을 학습할 수 있도록 준비시간 60초를 제공하고, 이후 점검시간은 25초로 줄여 운영 템포를 유지한다.

### 포트와 링크 용량

직접 구성의 App Server A는 입구 요청과 출구 응답 링크를 위해 트래픽 포트 2개를 사용한다. App Server B는 Load Balancer와 연결되는 양방향 링크 1개를 사용한다. DB 연결은 데이터 포트로 분리한다. Load Balancer는 입구, 출구, 두 서버를 연결하도록 트래픽 포트 4개를 가진다.

링크는 자유 선이 아니라 그리드 칸을 소비하는 인프라 자원이다. 링크 하나의 길이와 모든 링크의 합계가 LINK LEVEL 용량을 넘으면 연결하거나 장비를 이동할 수 없다. 수평 확장 구조는 LINK LV.2와 BOARD LV.2가 필요하도록 설계한다.

입구의 요청 링크는 파랑, 출구의 응답 링크는 보라, DB 데이터 링크는 노랑으로 표시한다. 내부 트래픽 링크는 같은 연결을 두 번 그리지 않고 파랑·보라 이중선으로 요청과 응답의 양방향 흐름을 표현한다.

보드는 7×4에서 시작하고 Credits로 10×5, 13×6까지 확장한다. 보드 확장은 새 장비를 놓을 수 있는 공간을, LINK LEVEL은 그 공간을 연결할 수 있는 거리와 총량을 제공한다.
