...
- Traffic Ingress: 트래픽 간선 1개
- App Server: 상위 트래픽 간선 1개와 DB 데이터 간선 1개
- Load Balancer: 트래픽 간선 3개
- Primary DB: 데이터 간선 2개
- 허용 경로 외의 장비 조합은 연결 불가

간선 길이는 두 장비 사이의 가로·세로 그리드 칸 수로 계산합니다.

| LINK LEVEL | 간선 1개 최대 | 전체 간선 예산 | 확장 비용 |
|---:|---:|---:|---:|
| 1 | 4칸 | 8칸 | 60 |
| 2 | 6칸 | 22칸 | 100 |
| 3 | 9칸 | 36칸 | MAX |

보드는 기존 9×4에서 더 촘촘한 13×6 그리드로 확장했습니다.

## 화면 배치

- 우측: 웨이브 정보, LINK 용량, 인프라 상점
- 하단 첫 줄: 서비스 개시 Progress Bar
- 하단 둘째 줄: 가로형 보유 장비 Dock과 배치 상태
